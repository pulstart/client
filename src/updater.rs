...
fn run_apply_update_command(command: &ApplyUpdateCommand) -> Result<(), String> {
    ...
    #[cfg(windows)]
    {
        if let Err(err) = sync_package_contents(&command.package_root, &command.install_root) {
...
fn archive_kind() -> Result<ArchiveKind, String> {
    match (std::env::consts::OS, std::env::consts::ARCH) {
        ...
        ("windows", "x86_64") | ("macos", "x86_64") | ("macos", "aarch64") => Ok(ArchiveKind::Zip),
        _ => Err(supported_target_label().unwrap_err()),
    }
}
...
fn current_install_target() -> Result<InstallTarget, String> {
    ...
    if cfg!(target_os = "macos") {
        let app_root = macos_app_bundle_root(&current_executable).ok_or_else(|| {
            "Full package update on macOS requires the client to run from a .app bundle."
                .to_string()
        })?;
        return Ok(InstallTarget {
            relaunch_executable: app_root.join("Contents/MacOS/st-client"),
...
fn spawn_elevated_helper_windows(
    ...
    let file = path_to_wide(helper_executable);
    ...
    if (result as isize) <= 32 {
        return Err(
            "Failed to launch elevated updater. The update requires administrator privileges."
                .to_string(),
        );
    }
...
fn wait_for_process_exit(pid: u32) -> Result<(), String> {
    ...
    Err("Timed out waiting for the running client process to exit.".to_string())
}
...
fn remove_or_rename_old(path: &Path) -> Result<(), String> {
    ...
    retry_io(rename_retries, rename_delay, || fs::rename(path, &old_path)).map_err(|err| {
        format!(
...
fn cleanup_old_files_in(dir: &Path) {
    ...
    for entry in entries.flatten() {
        ...
        } else if path.to_string_lossy().ends_with(".st-update-old") {
...
fn relaunch_updated_app(path: &Path) -> Result<(), String> {
    ...
    command.spawn().map_err(|err| {
        format!(
            "Failed to relaunch updated client '{}': {err}",
            path.display()
        )
    })?;
    Ok(())
...
fn http_get_response(url: &str, json: bool) -> Result<Box<dyn io::Read>, String> {
    ...
    let request = agent
        .get(url)
        .set("User-Agent", &format!("st-client/{}", current_version()));
    let request = if json {
