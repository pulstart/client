...
Packages target/release/st-client into a macOS .app bundle and, when signing
...
app_executable="$app_root/Contents/MacOS/st-client"
...
copy_app_bundle
python3 "${client_root}/scripts/bundle-macos-libs.py" "$binary_path" "$app_executable"
...
if [[ "$reported_version" != "st-client $version" ]]; then
